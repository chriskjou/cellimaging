...
if not is_training:
    ...
    print("TOTAL TEST FILES: " + str(testfiles))
    # adds subfolders for subimages
    for each in testfiles:
        ...
        print(where)
        screenshot(where)
        temp = glob.glob(where + "subimages"+ "/*")
        subtestfiles.extend(temp)

    for each in subtestfiles:
...
print('reading ', len(sizes_test), ' of ', len(subtestfiles))
...
actual["infected"] = cellcounter
# sub['EncodedPixels'] = pd.Series(rles).apply(lambda x: ' '.join(str(y) for y in x))

title = datetime.now().strftime('%Y-%m-%d=%H-%M-%S')
actual.to_csv(title + '.csv', index=False)
